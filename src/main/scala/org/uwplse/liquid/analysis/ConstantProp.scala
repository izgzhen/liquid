...
class ConstantProp (val icfg: InterproceduralCFG[soot.Unit, SootMethod])
  extends
    DefaultJimpleIFDSTabulationProblem[Pair[Value, Set[ConstVal]], InterproceduralCFG[soot.Unit, SootMethod]](icfg) {
  type abstractionMap = mutable.Map[soot.Unit, mutable.TreeMap[Value, Set[ConstVal]]]
  private def initAbstractionMap() = new mutable.TreeMap[soot.Unit, mutable.TreeMap[Value, Set[ConstVal]]]()(Ordering.by(_.toString()))
  ...
  private val id = Identity.v[Pair[Value, Set[ConstVal]]]()
  private val killAll = KillAll.v[Pair[Value, Set[ConstVal]]]()

  private def putUnitAbstractions(u: soot.Unit, value: Value, abstractions: Set[ConstVal]): Boolean = {
    unitAbstractionMap.getOrElseUpdate(u, new mutable.TreeMap[Value, Set[ConstVal]]()(Ordering.by(_.toString()))).addOne(value, abstractions)
    visitedMethods.add(interproceduralCFG.getMethodOf(u))
  }

  private def putUnitAbstractionsAfter(u: soot.Unit, flow: Iterable[Pair[Value, Set[ConstVal]]]): Boolean = {
    for (pair <- flow) {
      unitAbstractionAfterMap
        .getOrElseUpdate(u, new mutable.TreeMap[Value, Set[ConstVal]]()(Ordering.by(_.toString())))
        .addOne(pair.getO1, pair.getO2)
    }
    visitedMethods.add(interproceduralCFG.getMethodOf(u))
  }

  private def getInterestingConstantAbstraction(v: Value): Set[ConstVal] = {
  ...
  override def createFlowFunctionsFactory: FlowFunctions[soot.Unit, Pair[Value, Set[ConstVal]], SootMethod] = new FlowFunctions[soot.Unit, Pair[Value, Set[ConstVal]], SootMethod]() {
    override def getNormalFlowFunction(curr: soot.Unit, successor: soot.Unit): FlowFunction[Pair[Value, Set[ConstVal]]] = {
      if (curr.isInstanceOf[DefinitionStmt]) {
        val assignment = curr.asInstanceOf[DefinitionStmt]
        (source: Pair[Value, Set[ConstVal]]) => {
          def foo(source: Pair[Value, Set[ConstVal]]): Set[Pair[Value, Set[ConstVal]]] = {
            ...
            if (!(source == zeroValue)) {
              ...
              if (source.getO1.equivTo(assignment.getRightOp)) return Set(new Pair[Value, Set[ConstVal]](assignment.getLeftOp, source.getO2))
              newFlow.add(source)
              ...
              if (constants.nonEmpty) {
                Set(new Pair[Value, Set[ConstVal]](assignment.getLeftOp, constants))
              } else {
                Set()
              }
            }
    ...
    override def getCallFlowFunction(callStmt: soot.Unit, destinationMethod: SootMethod): FlowFunction[Pair[Value, Set[ConstVal]]] = {
        ...
        def foo(source: Pair[Value, Set[ConstVal]]): Set[Pair[Value, Set[ConstVal]]] = {
          ...
          if (source ne zeroValue) {
            ...
            if (args.contains(source.getO1)) {
              ...
              val pair = new Pair[Value, Set[ConstVal]](new EquivalentValue(Jimple.v.newParameterRef(destinationMethod.getParameterType(paramIndex), paramIndex)), source.getO2)
              newFlow.add(pair)
            }
          }
          else {
            for (arg <- args.asScala) {
              if (!arg.isInstanceOf[Local]) {
                ...
                if (constants.nonEmpty) {
                  ...
                  val pair = new Pair[Value, Set[ConstVal]](new EquivalentValue(Jimple.v.newParameterRef(destinationMethod.getParameterType(paramIndex), paramIndex)), constants)
                  newFlow.add(pair)
                }
              }
            }
    ...
    override def getReturnFlowFunction(callSite: soot.Unit, calleeMethod: SootMethod, exitStmt: soot.Unit, returnSite: soot.Unit): FlowFunction[Pair[Value, Set[ConstVal]]] = {
        ...
        def foo(source: Pair[Value, Set[ConstVal]]): Set[Pair[Value, Set[ConstVal]]]  = {
          ...
          if (exitStmt.isInstanceOf[ReturnStmt]) {
            ...
            if (returnStmt.getOp.equivTo(source.getO1)) {
              ...
              val pair = new Pair[Value, Set[ConstVal]](definitionStmt.getLeftOp, source.getO2)
    ...
    override def getCallToReturnFlowFunction(callSite: soot.Unit, returnSite: soot.Unit): FlowFunction[Pair[Value, Set[ConstVal]]] = { // it is just that definition might reach from things not relevant to the current call, just like
  ...
  override def initialSeeds: java.util.Map[soot.Unit, java.util.Set[Pair[Value, Set[ConstVal]]]] = {
  ...
  override def createZeroValue = new Pair[Value, Set[ConstVal]](new JimpleLocal("<<zero>>", NullType.v), Set())
}
